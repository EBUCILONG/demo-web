...
merger_log_path = os.environ['GRASPER_MERGE_LOG_PATH']
worker_log_path = os.environ['GRASPER_LOG_PATH']
...
def runApplication():
    ...
    merger_cmd = 'mpiexec -n {} -f machines.cfg python utils/grasper-demo-coordinator-mpi.py -t {}'.format(num_worker, timestamp)
    ...
    # 2. run grasper
    cmd, ini_str = utils.ini_generator.grasper_ini_gen(data)
    tmpf_dir = os.path.join(myenv['GRASPER_HOME'], 'tmp')
    if not os.path.exists(tmpf_dir):
        os.mkdir(tmpf_dir)
    myenv['GRASPER_INI_NAME'] = os.path.join(myenv['GRASPER_HOME'], 'tmp', str(timestamp)+'.ini')

    myenv['GRASPER_START_TIMESTAMP'] = timestamp
    with open(myenv['GRASPER_INI_NAME'], 'w') as f:
        f.write(ini_str)

    print('run command: ', cmd)
    final_cmd = 'GRASPER_INI_NAME={} GRASPER_START_TIMESTAMP={} {}'.\
            format(myenv['GRASPER_INI_NAME'], myenv['GRASPER_START_TIMESTAMP'], cmd)
